...
def update_output(contents, filename, date):
    if contents is not None:
        ...
        gender_fig = px.pie(df, names='Пол', title='Мужчина-Женщина', color='Пол', color_discrete_map=color_map)
        age_fig = px.histogram(df, x='Возрастная группа', title='Возраст', color_discrete_sequence=age_color)
        day_fig = px.histogram(df, x='День недели', title='Дни недели', category_orders={'День недели': ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье']})
        amount_fig = px.histogram(df, x='Возрастная группа', y='Потраченная сумма', color='Пол', barmode='group', title='Потраченная сумма по возрасту и полу', color_discrete_map=color_map)
